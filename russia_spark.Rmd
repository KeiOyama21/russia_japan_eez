...
# 引っ張ってくるテーブルを指定
sql <- "SELECT * FROM `fish-database-275116.GFW_Data.big_area_segs` where ssvid like '440%'or ssvid like'441%'"
# クエリの実行および取得データをtibbleとして保存
tb <- bq_project_query(projectid, sql)
# テーブルをダウンロード
segs_korea <- bq_table_download(tb) %>% 
  distinct() %>% 
  arrange(seg_id)
```
```{r}
...
print(unique(russia_eez_japan_korea_2018_trwalers$ssvid))
```
...
russia_eez_japan_2018_trwalers_sf <- st_as_sf(russia_eez_japan_2018_trwalers, coords = c("avg_lat", "avg_lon"), crs = 4326)
