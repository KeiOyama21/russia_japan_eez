...
russia_eez_japan_2018_trwalers <- dat_area_segs_daily_WE_2018 %>%
  ...
  left_join(dat_sonota_joined, by = c("first_timestamp", "seg_id")) %>% 
  # territory1 が "Japan" の行のみフィルタ
  filter(reporting_name == "Japanese Exclusive Economic Zone") %>% 
  #select(seg_id, ssvid, first_timestamp, eez_id, reporting_name) %>% 
   mutate(avg_lon = substring_index(avg_lat_lon, " ", 1),#avg_lat_lonをavg_lon,avg_latに分ける
          avg_lat = substring_index(avg_lat_lon, " ", -1),
          seg_id_num  = as.numeric(str_sub(seg_id, start = 1L, end = -1L)))%>%
...
#漁船のssvidを抜き出す ssvid_fishing_gear, trawlersのssvid ssvid_trwalersを出す
```{r}
...
ssvid_trawlers <- segs_russia_vessel_type %>% 
  filter(best_vessel_class %in% "trawlers") %>% 
  pull(ssvid)
...
```{r}
# 2018年
catch_fish_cleaned_russia_eez_japan_2018 <- russia_eez_2018%>%
  filter(
    ssvid %in% (
      intersect(
    ...
    ssvid %in% ssvid_fishing_gear
  )

# 2019年
catch_fish_cleaned_russia_eez_japan_2019 <- russia_eez_2019 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
...
# 2020年
catch_fish_cleaned_russia_eez_japan_2020 <- russia_eez_2020 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
...
# 2021年
catch_fish_cleaned_russia_eez_japan_2021 <- russia_eez_2021 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
...
# 2022年
catch_fish_cleaned_russia_eez_japan_2022 <- russia_eez_2022 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
...
# 2023年
catch_fish_cleaned_russia_eez_japan_2023 <- russia_eez_2023 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
...
# lapplyでループ処理
lapply(data_list, function(data_info) {
  # データ名と年を取得
...
data_list <- lapply(years, function(year){
  list(
    data_name = paste0("monthl,fishing_hour_", year),
    year = year
  )
})

lapply(data_list, function(data__info){
...
#2018 ~ 2023年のdailyデータをロシア船+日本のeezで操業+韓国に行っている+トロール漁船 にフィルタリング
```{r}
# 2018年
trawlers_russia_2018 <- russia_eez_2018 %>%
  filter(
    ssvid %in% (
      intersect(
        ssvid[fishing_hours > 0 & territory1 == "Japan"],
    ...
    ssvid %in% ssvid_trawlers
  ) %>%
  left_join(
    segs_russia_vessel_type %>% distinct(ssvid, .keep_all = TRUE), 
    by = "ssvid"
  )
length(unique(trawlers_russia_2018$ssvid))

trawlers_russia_list <- lapply(years, function(year) {
  # 対応する年のデータフレームを動的に取得
  russia_eez_data <- get(paste0("russia_eez_", year))
